...
#尝试：1 直接索引neuron_index对应的model.parameters到optimizer中,失败
#2 把.detach.requires_grad(True) 改为 nn.Parameters(xxx)，失败
#3 contrast experiments中的parameter list换成字典，仍然成功
...
class MNISTNet2(MetaModule):
    ...
    def neuron_params(self, layer_name, neuron_idx):
        ...
        key_series = pd.Series(net.list_named_parameters.keys())
        index = key_series[key_series.values == layer_name].index[0]
        if neuron_idx == -1:
            return list(list(self.parameters())[index])
        else:
            return list(list(self.parameters())[index][:,neuron_idx]) #size(next_neuron_numbers,last_neuron_numbers)
...
def init_optimizers2(net):
    ...
    optimizers["x"] = torch.optim.Adam([x.detach().requires_grad_(True) for x in list_param], lr=0.001)
    return optimizers
